...
router.get("/stats/dashboard", authenticateToken, (req, res) => {
  try {
    ...
    const activeApplications = dbGet(
      'SELECT COUNT(*) as count FROM grants WHERE status IN ("APPLYING","SUBMITTED")',
    ).count;
    const pendingReview = dbGet(
      'SELECT COUNT(*) as count FROM grants WHERE status = "REVIEWING"',
    ).count;
    const fundedGrants = dbGet(
      'SELECT COUNT(*) as count FROM grants WHERE status = "FUNDED"',
    ).count;
    const totalFunding = dbGet(
      'SELECT COALESCE(SUM(amount_max), 0) as total FROM grants WHERE status = "FUNDED"',
    ).total;
    const upcomingDeadlines = dbGet(
      'SELECT COUNT(*) as count FROM grants WHERE deadline > date("now") AND deadline <= date("now", "+30 days")',
    ).count;
    const aiMatches = dbGet(
      'SELECT COUNT(*) as count FROM grants WHERE relevance_score = "HIGH"',
    ).count;
    const submitted = dbGet(
      'SELECT COUNT(*) as count FROM grants WHERE status IN ("SUBMITTED","FUNDED","REJECTED")',
    ).count;
    const successRate =
      submitted > 0 ? Math.round((fundedGrants / submitted) * 100) : 0;
...
router.get("/", authenticateToken, (req, res) => {
  try {
    ...
    if (search) {
      query +=
        " AND (name LIKE ? OR description LIKE ? OR organization LIKE ?)";
...
router.patch("/:id/status", authenticateToken, (req, res) => {
  try {
    ...
    dbRun(
      'UPDATE grants SET status = ?, updated_at = datetime("now") WHERE id = ?',
