...
function generateFallbackDeadline(relevanceScore) {
  ...
  let daysAhead =
    relevanceScore === "HIGH" ? 75 : relevanceScore === "MEDIUM" ? 105 : 135;
  // Add some randomness
...
async function scrapeFundsForNGOs() {
  try {
    const { data } = await axios.get(
      "https://fundsforngos.org/tag/education/",
      {
        timeout: 15000,
    ...
    $("article, .post")
      ...
      .each((_, el) => {
        ...
        const excerpt = $(el)
          .find(".entry-summary, .entry-content p")
          .first()
          .text()
          .trim();
  ...
  try {
    ...
    $("article, .post")
      ...
      .each((_, el) => {
        ...
        const link = $(el).find("h2 a, .entry-title a").attr("href");
        const excerpt = $(el)
          .find(".entry-summary p, .entry-content p")
          .first()
          .text()
          .trim();
...
async function scrapeUNDP() {
  try {
    const { data } = await axios.get(
      "https://www.undp.org/content/dam/undp/library/corporate/opportunities/grants.json",
      {
        timeout: 15000,
        headers: { "User-Agent": USER_AGENT },
      },
...
async function scrapeWorldBank() {
  try {
    const { data } = await axios.get(
      "https://projects.worldbank.org/en/projects-operations/document-search",
      {
        timeout: 15000,
        headers: {
...
async function scrapeUKRI() {
  try {
    const { data } = await axios.get(
      "https://www.ukri.org/opportunity/opportunities/",
      {
        timeout: 15000,
        headers: { "User-Agent": USER_AGENT },
      },
...
async function scrapeEuropeanEducation() {
  try {
    const { data } = await axios.get(
      "https://ec.europa.eu/info/funding-tenders/opportunities/data/erasmus-plus_en",
      {
        timeout: 15000,
        headers: { "User-Agent": USER_AGENT },
      },
...
async function scrapeGrantsGov() {
  try {
    const { data } = await axios.post(
      "https://grantsapi.com/api/v2/opportunities/search",
    ...
    return data.opportunities.map((opp) => ({
      ...
      website_url:
        opp.url || `https://grants.gov/search-results-detail/${opp.oppNumber}`,
      source: "Grants.gov",
    }));
  } catch (err) {
  ...
  try {
    ...
    function extractAmount(text) {
      ...
      const match = combined.match(
        /(\$|€|£|₦)\s?(\d{1,3}(?:,\d{3})*(?:\.\d+)?)|(\d{1,3}(?:,\d{3})*(?:\.\d+)?)\s?(usd|eur|gbp|ngn)/i,
      );
    ...
    for (const g of allGrants) {
      ...
      const validatedDeadline = deadlineValidation.valid
        ? deadlineValidation.formatted
        : null;

      // Use fallback deadline if none found (for education grants, assign reasonable future date)
      const finalDeadline =
        validatedDeadline ||
        (relevance !== "LOW" ? generateFallbackDeadline(relevance) : null);

      // Check if grant is already expired
      const isExpired = finalDeadline
        ? new Date(finalDeadline) < new Date()
          ? 1
          : 0
        : 0;
      ...
      const existing = dbGet(
        "SELECT id FROM grants WHERE name = ? AND source = ?",
        [g.name, g.source],
      );
      if (existing) continue;

      dbRun(
        `INSERT INTO grants (name, organization, description, category, relevance_score,
    ...
    console.log(
      `[Scraper] Complete: ${totalFound} found, ${totalMatched} new matches stored`,
    );
